document.addEventListener('DOMContentLoaded', async () => {
    ...
    const availableLixosTab = document.getElementById('availableLixosTab');
    const myCollectionsTab = document.getElementById('myCollectionsTab');

    // Seções
    const dashboardSection = document.getElementById('dashboardSection');
    const availableLixosSection = document.getElementById('availableLixosSection');
    const myCollectionsSection = document.getElementById('myCollectionsSection');

    // Lixos Disponíveis (Coletar Lixo)
    ...
    // Inicializa alguns lixos se estiver vazio para simulação (n ta funcionando no localstorage)
    ...
    function showTab(tabId) {
        ...
        document.querySelectorAll('.tab-button').forEach(button => {
            ...
            if (button.classList.contains('active')) {
                img.style.filter = 'brightness(0) saturate(100%) invert(75%) sepia(88%) saturate(1987%) hue-rotate(352deg) brightness(101%) contrast(105%)';
            } else {
    ...
    //  SVG do ícone baseado no tipo de lixo caso nao carregue a imagem normal
    ...
    // ---Carregar Lixos Disponíveis(Coletar Lixo) ---
    async function loadAvailableLixos() {
        ...
        filteredLixos.forEach(lixo => {
            ...
            card.className = `lixo-card status-Pendente`;
    ...
    // --- Carregar Minhas Coletas  (Status) ---
    ...
    function renderRecicladorCharts(myCollectedLixos) {
        ...
        if (coletadoSemanaChartInstance) {
            coletadoSemanaChartInstance.destroy();
        }
        ...
        if (coletadoMesChartInstance) {
            coletadoMesChartInstance.destroy();
        }
        coletadoMesChartInstance = new Chart(ctxMes, {
            ...
            data: {
                ...
                datasets: [{
                    ...
                    backgroundColor: 'rgba(3, 140, 127, 0.4)',
                    borderColor: '#038C7F',
                    borderWidth: 2,
                    tension: 0.3,
    ...
    async function loadDashboardStats() {
        ...
        const lixoColetadoRecentemente = myCollectedLixos.sort((a, b) => new Date(b.data_coleta) - new Date(a.data_coleta));
        ...
        renderRecicladorCharts(myCollectedLixos); // Chart.js

        // Carregar os cards de lixo coletado no dashboard
